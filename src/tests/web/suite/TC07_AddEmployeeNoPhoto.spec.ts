...
test.describe('TC07 - Add Employee Without Photo', () => {
  ...
  test('Add Employee without photo successfully', async ({ page }) => {
    await dashboardPage.clickPIM();
    await pimPage.clickAddEmployee();
    
    const id = `ID_${Date.now().toString().slice(-4)}`;
    await pimPage.fillEmployeeDetails('NoPhoto', 'QA', 'User', id);
